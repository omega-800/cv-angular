    ...
    if(projects && ((skillFilters && skillFilters.length > 0) || (projectFilters && projectFilters.length > 0))) {
      return projects.filter(project => {
        ...
        projectFilters.forEach(filter => {
          if(!applies){
            applies = filter.category === "filter_project_link" ? (filter.value == linkTypes.URL.id ? project.url != "" : filter.value == linkTypes.GITHUB.id ? project.github != "" : false) :
              filter.category === "filter_project_author" ? project.authors.some(author => author.person_id == filter.value) :
              filter.category === "filter_project_career" ? project.career !== undefined && project.career.career_id == filter.value :
              filter.category === "filter_project_client" ? ((project.client !== undefined && project.client.person_id == filter.value) || (project.clients.some(client => client.client_id == filter.value))) : false;
          }
        });
        return applies;
      });
