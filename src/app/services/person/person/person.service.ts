...
export class PersonService {
  ...
  private personCountryLink: Person_Country[] = (person_countryData as any)
    .default;
  ...
  getPersonByName(name: string): PersonEntity {
    return Object.values(this.people).filter(
  ...
  fillPerson(person: PersonOnly): PersonEntity {
    let address: AddressEntity = this.addressService.getAddressById(
      person.address_id
    );
    let workplace: WorkplaceEntity = this.workplaceService.getWorkplaceById(
      person.workplace_id
    );
    let school: SchoolEntity = this.schoolService.getSchoolById(
      person.school_id
    );
    let personcategory: PersonCategoryEntity =
      this.personCategoryService.getPersonCategoryById(
        person.personcategory_id
      );
    let contact: ContactEntity = this.contactService.getContactById(
      person.contactpoint_id
    );
    let countries: CountryEntity[] = this.personCountryLink
      .filter((link) => link.person_id === person.person_id)
      .map((link) => this.countryService.getCountryById(link.country_id));
    return {
      ...person,
      id: 'person_' + person.person_id,
      ...
      countries: countries,
      thumbnail: {
        id: 'thumbnail_' + person.person_id,
        name: 'Thumbnail ' + person.firstname + ' ' + person.lastname,
        alt: 'Thumbnail ' + person.firstname + ' ' + person.lastname,
        path: person.thumbnail,
      },
      image: {
        id: 'image_' + person.person_id,
        name: 'Image of ' + person.firstname + ' ' + person.lastname,
