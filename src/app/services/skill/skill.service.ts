...
import { Skill, SkillCategory, SkillNew, SkillOnly, SkillSubCategory, SkillSubCategoryOnly, SkillTypes, Skill_SkillSubCategory } from './skill.model';
...
export class SkillService {
  ...
  constructor(skillCategoriesService:SkillCategoriesService, applicationService:ApplicationService, languageService:LanguageService, knowledgeService:KnowledgeService, activityService:ActivityService, abilityService:AbilityService) { 
    ...
    this.skillsNew = this.onlySkills.map(skill =>this.fillSkillPropsNew(skill, skillCategoriesService, applicationService, languageService, knowledgeService, activityService, abilityService));
  }
  ...
  fillSkillPropsNew(skill:SkillOnly, skillCategoriesService:SkillCategoriesService, applicationService:ApplicationService, languageService:LanguageService, knowledgeService:KnowledgeService, activityService:ActivityService, abilityService:AbilityService):SkillNew {
    if(skill.application_id !== ""){ 
      return {...skill, skillsubcategories:skillCategoriesService.getSkillSubCategoriesOfSkill(skill), ...applicationService.getApplicationById(skill.application_id), type:SkillTypes.app}; 
    }
    if(skill.language_id !== ""){
      return {...skill, skillsubcategories:skillCategoriesService.getSkillSubCategoriesOfSkill(skill), ...languageService.getLanguageById(skill.language_id), type:SkillTypes.sprache}; 
    }
    if(skill.knowledge_id !== ""){  
      return {...skill, skillsubcategories:skillCategoriesService.getSkillSubCategoriesOfSkill(skill), ...knowledgeService.getKnowledgeById(skill.knowledge_id), type:SkillTypes.wissen};
    }
    if(skill.activity_id !== ""){ 
      return {...skill, skillsubcategories:skillCategoriesService.getSkillSubCategoriesOfSkill(skill), ...activityService.getActivityById(skill.activity_id), type:SkillTypes.aktivität};
    }
    if(skill.ability_id !== ""){  
      return {...skill, skillsubcategories:skillCategoriesService.getSkillSubCategoriesOfSkill(skill), ...abilityService.getAbilityById(skill.ability_id), type:SkillTypes.skill};
    }
