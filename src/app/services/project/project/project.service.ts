...
import { lockIcon } from 'src/app/components/icons.constants';
...
export class ProjectService {
  ...
  constructor(
    ...
    this.projects = this.onlyProjects.map((project) =>
  ...
  fillProject(project: ProjectOnly, images: ImageComp[]): ProjectEntity {
    return {
      ...
      authors: this.projectAuthorLinks
        .filter((link) => link.project_id == project.project_id)
        .map((link) => this.personService.getPersonById(link.author_id)),
      skills: this.projectSkillLinks
        .filter((link) => link.project_id == project.project_id)
        .map((link) => this.skillService.getSkillById(link.skill_id)),
      clients: this.projectClientLinks
        .filter((link) => link.project_id == project.project_id)
      ...
      thumbnail: {
        id: 'thumbnail_' + project.project_id,
        name: 'Thumbnail of ' + project.name,
